...
def getvideoduration(vidfile):
    cmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffprobe.exe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 %s"%vidfile
    try:
...
def getaudioduration(audfile):
    cmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffprobe.exe -v error -select_streams a:0 -show_entries stream=duration -of default=noprint_wrappers=1:nokey=1 %s"%audfile
    try:
        outstr = subprocess.check_output(cmd, shell=True)
    except:
        print("Could not find the duration of '%s' - Error: %s"%(vidfile, sys.exc_info()[1].__str__()))
...
if __name__ == "__main__":
    ...
    if deltafile is not None:
        ...
        audclipcmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffmpeg.exe -ss 00:00:00 -i %s -c:a copy -to 00:%s:%s %s"%(deltafile, tmin, tsec, clippedaudiofile)
        print(audclipcmd)
    ...
    aconcatcmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffmpeg.exe -y %s -filter_complex %sconcat=n=%s:v=0:a=1[out] -map [out]? concataudio.wav"%(minusistr, filterstr, n)
    print(aconcatcmd)
    ...
    if os.path.exists("concataudio.wav"): # Concatenation was successful, so do the overlay of the video with this audio file
        overlaycmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffmpeg.exe -y -i %s -i concataudio.wav -map 0 -map 1 -c copy %s"%(videofile, outvideo)
        #overlaycmd = "C:\\ffmpeg\\ffmpeg-2022-09-05-git-2ed5925e26-full_build\\bin\\ffmpeg.exe -y -i %s -i concataudio.wav -c:v copy -c:a aac -map 0:v:0 -map 1:a:0 %s"%(videofile, outvideo)
...
"""
How to run:
python clipmerge_win.py video_file_path audio_dir_path audio_ext
